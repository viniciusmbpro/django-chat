...
class ChatGetInView(LoginRequiredMixin, View):
    ...
    def post(self, request, id):
        ...
        if ChatParticipant.objects.filter(chat=chat, user=request.user).exists():
            return render(request, 'chat/pages/chat-view.html', {'chat': chat, 'error_message': 'Você já é participante deste chat.'})
...
class ChatAddMessageView(LoginRequiredMixin, View):
    ...
    def post(self, request, id):
        ...
        if not ChatParticipant.objects.filter(chat=chat, user=request.user).exists():
            return render(request, 'chat/pages/chat-view.html', {'chat': chat, 'error_message': 'Você não é participante deste chat.'})
